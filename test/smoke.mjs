...
const SCHEMA_PATH = fileURLToPath(new URL("../d1/schema.sql", import.meta.url));
const LOCAL_TOKEN = `local-smoke-${process.pid}-${Date.now()}`; // 仅本地桩运行时生成，非凭据
const WRONG_TOKEN = `wrong-smoke-${process.pid}-${Date.now()}`;
const BOGUS_SECRET = `bogus-smoke-${process.pid}-${Date.now()}`;
...
// S. 错误令牌负例 → 可读中文错误（401/302 归一化，v1.2 保留）
{
  ...
  if (TARGET === "local") {
    bad = new SagittaMemoryClient({
      ...
      envPath: "unused",
      timeoutMs: 10000,
      auth: { accessClientId: "", accessClientSecret: "", authToken: WRONG_TOKEN, accessPresent: false, bearerPresent: true },
    });
  } else {
    bad = new SagittaMemoryClient(resolveConfig({
      accessClientId: "bogus-smoke-id",
      accessClientSecret: BOGUS_SECRET,
      authToken: "",
    }));
  }
