...
配置字段以 `lib/config.js` 的 Schemastery schema 为准。旧版 `path`、`branch`、`repos` 仍兼容；新配置使用 `repoPath`、`profile`、`presetId`、`workerUpdate` 和 `restartPolicy`。默认分支是 `main`、默认重启策略是 `prompt`、默认 Worker 更新开关为 `true`。`install-profile-deps.ps1` 对已有 profile patch 中的本机路径（`repoPath/path`、`presetTarget`、`profileDir`）以及 updater 明确允许 profile 覆盖的 `branch`、`restartPolicy`、`workerDeploy` 保持现值优先；只有缺失时才写入与代码默认一致的值。磁盘更新后默认提示手动执行 `dsh --profile web`。
